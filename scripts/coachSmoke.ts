...
//
// Higiene: jamas imprimir valores de env ni headers de request. Los errores
// del endpoint ya salen sanitizados por api/_utils.ts (publicServerError).
...
async function main(): Promise<number> {
  ...
  try {
    ...
    console.log(`[coach-smoke] POST /api/coach-agent (timeout ${REQUEST_TIMEOUT_MS / 1000}s)...`);
    ...
    try {
      ...
      console.error(
        isTimeout
          ? `FAIL: /api/coach-agent no respondio en ${REQUEST_TIMEOUT_MS / 1000}s`
