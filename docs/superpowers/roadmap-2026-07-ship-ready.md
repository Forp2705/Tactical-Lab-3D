...
**Scope ship-ready.**
- **Line endings (ÚLTIMA mutación — después de junk Y del cherry-pick de Gemini):** las mutaciones semánticas van antes de la normalización EOL, así renormalize es una única pasada estructural LF-only y se evita el conflicto de EOL en el cherry-pick. `.gitattributes` con `* text=auto eol=lf` **+ reglas binarias explícitas** (`*.glb *.png *.jpg *.jpeg *.gif *.webp *.ico *.woff *.woff2 *.ttf *.zip *.mp4 *.mov *.pdf` → `binary`), luego `git add --renormalize .` en un commit dedicado. Diff grande pero semánticamente vacío, one-time; después la tree queda limpia y el churn CRLF no puede recurrir. Las reglas binarias garantizan que renormalize nunca toque un `.glb`/PNG/asset. **Co-locar `biome.json` `lineEnding: lf`** en la misma unidad — `.gitattributes` y biome deben coincidir en LF o el churn vuelve (era el landmine a tres bandas).
